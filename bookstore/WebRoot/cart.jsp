...
<script type="text/javascript">
	function change(id,num,pnum){
		...
		location.href="${pageContext.request.contextPath}/changCount?id="+id+"&num="+num;
	}
</script>
								...
								<table width="100%" border="0" cellspacing="0">
									...
									<tr>
										<td>
											...
											<c:forEach var="cart" items="${cart}" varStatus="c">
												...
												<table width="100%" border="0" cellspacing="0">
													...
													<tr>
														...
														<input type="button" value='-'
															style="width:20px" onclick="change('${cart.key.id}','${cart.value-1 }','${cart.key.pnum }')">

															<input name="text" type="text"  value="${cart.value }"
															style="width:40px;text-align:center" /> <input
															type="button" value='+' style="width:20px" onclick="change('${cart.key.id}','${cart.value+1 }','${cart.key.pnum }')">

														</td>
														<td width="10%">${cart.key.pnum }</td>
														<td width="10%">${cart.key.pnum*cart.value }</td>

														<td width="10%"><a href="${pageContext.request.contextPath}/changCount?id=${cart.key.id}&num=0"
															style="color:#FF0000; font-weight:bold">X</a></td>
													</tr>
													
												</table>
													<c:set var="sum" value="${sum+cart.key.pnum*cart.value }"></c:set>
												</c:forEach>
